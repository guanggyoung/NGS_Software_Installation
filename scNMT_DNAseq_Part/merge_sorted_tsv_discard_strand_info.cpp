...
struct TSV_entry
{
	...
	friend std::ostream & operator << (std::ostream & TSV_output,  TSV_entry & one_TSV_line)
	{
        ...
        // TSV_output<<one_TSV_line.strand<<'\t'; //discard strand info
...
void merge_same_base_in_TSV(std::string TSV_filename)
{
    ...
    std::ofstream updated_TSV(TSV_filename+"_merged_strand_merged.tsv");
    // Write the new bed file with sequence info into a new file.
    std::ofstream updated_bed(TSV_filename+"_merged_strand_merged.bed");
    // print table head
    updated_TSV<<"ref"<<'\t'<<"pos"<<'\t'<<"methylated"<<'\t'<<"un_methylated"<<'\t'<<"met_ratio"<<std::endl;
    updated_bed<<"ref"<<'\t'<<"start"<<'\t'<<"end"<<'\t'<<"met_ratio"<<std::endl;
	...
	while( std::getline(TSV_input, one_string_line) )
	{
        ...
        if(one_TSV_line.ref_chr_name == old_TSV_line.ref_chr_name &&
           one_TSV_line.position == old_TSV_line.position) // same base in the genome, merge the new line to old
        {
        ...
        {   
            ...
            updated_bed<<old_TSV_line.ref_chr_name<<'\t'<<old_TSV_line.position<<'\t'<<old_TSV_line.position<<'\t'
                       <<old_TSV_line.met_ratio<<std::endl;
